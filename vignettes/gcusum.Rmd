...
vignette: >
  %\VignetteIndexEntry{GCUSUM charts}
...
This vignette describes how to use GCUSUM charts, an extension to standard CUSUM charts for binary performance data grouped in samples of unequal size. 
...
```{r}
head(ragcusum_example_data)

```

## Non-risk-adjusted GCUSUM chart
...
```{r}
failure_probability <- mean(gcusum_example_data$y[gcusum_example_data$year == 2016])

n_patients <- nrow(gcusum_example_data[gcusum_example_data$year == 2016,])
```
...
GCUSUM charts are constructed on performance data from 2017.
```{r}

gcusum_data <- gcusum_example_data[gcusum_example_data$year == 2017,]

input_outcomes <- matrix(c(gcusum_data$y, gcusum_data$block_identifier), ncol = 2)


gcs <- gcusum(input_outcomes = input_outcomes,
...
racusum_limit <- racusum_limit_sim(patient_risks = ragcusum_example_data$score[ragcusum_example_data$year == 2016],
                            odds_multiplier = 2,
                            n_simulation = 1000,
                            alpha = 0.05,
                            seed = 2046)
...
ragcusum_data <- ragcusum_example_data[ragcusum_example_data$year == 2017,]
