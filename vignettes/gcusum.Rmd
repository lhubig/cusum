...
This vignette describes how to use GCUSUM charts, an extension to standard CUSUM charts for binary performance data grouped in sampels of unequal size. 
...
Like in the standard CUSUM chart (vgl. vignette for CUSUM charts), parameters have to be estimated in order to set up the charts,  
...
gcusum_data$blocks <- gcusum_data %>% group_indices(block_identifier)

input_outcomes <- matrix(c(gcusum_data$y, gcusum_data$blocks), ncol = 2)
...
ggplot() +
  ...
  scale_y_continuous(name = "Signal Probability")+
  theme_bw()
```
